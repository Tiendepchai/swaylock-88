...
static int parse_options(int argc, char **argv, struct swaylock_state *state,
  ...
  static struct option long_options[] = {
      ...
      {"indicator-anim-duration", required_argument, NULL,
       LO_INDICATOR_ANIM_DURATION},
      {"indicator-anim-intensity", required_argument, NULL,
       LO_INDICATOR_ANIM_INTENSITY},
      {0, 0, 0, 0}};
  ...
  const char usage[] =
      ...
      "  -S, --screenshots                "
      "Use screenshots of the current outputs as the background image.\n"
      "  --effect-blur <radius>           "
      "Apply a box blur approximation to the background image.\n"
      "  --effect-pixelate <factor>       "
      "Pixelate the background image.\n"
      "  --effect-fade-in <ms>            "
      "Fade in the background image over the given duration.\n"
      ...
      "  --indicator-anim <mode>          "
      "Sets the indicator animation style (none, pulse, spin, pulse+spin, "
      "ripple).\n"
      "  --indicator-anim-duration <ms>   "
      "Sets the duration of the indicator animation.\n"
      "  --indicator-anim-intensity <val> "
      "Sets the intensity of the indicator animation (0.0 - 1.0).\n"
      ...
      "  --battery                        "
      "Display the current battery percentage and charging status.\n"
      "  --clock                          "
      "Display a clock with the current time and date.\n"
      "  --timestr <format>               "
      "Format string for the time widget.\n"
      "  --datestr <format>               "
      "Format string for the date widget.\n"
      "  --show-user                      "
  ...
  while (1) {
    ...
    case LO_INDICATOR_ANIM_DURATION:
      if (state)
        state->args.indicator_anim_duration = strtoul(optarg, NULL, 0);
      break;
    case LO_INDICATOR_ANIM_INTENSITY:
      if (state)
        state->args.indicator_anim_intensity = strtod(optarg, NULL);
      break;
    default:
...
static void handle_screencopy_buffer(void *data,
  ...
  struct swaylock_surface *surface = data;
  create_buffer(surface->state->shm, &surface->screencopy_buffer, width, height,
                format);
...
static void handle_screencopy_ready(void *data,
  ...
  cairo_surface_t *img = cairo_image_surface_create(
      CAIRO_FORMAT_ARGB32, surface->screencopy_buffer.width,
      surface->screencopy_buffer.height);
  ...
  if (state.args.screenshots && state.screencopy_manager) {
    wl_list_for_each(surface, &state.surfaces, link) {
      ...
      zwlr_screencopy_frame_v1_add_listener(
          surface->screencopy_frame, &screencopy_frame_listener, surface);
    }
