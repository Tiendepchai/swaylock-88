...
static void schedule_input_idle(struct swaylock_state *state) {
  ...
  state->input_idle_timer =
      loop_add_timer(state->eventloop, 1500, set_input_idle, state);
}
...
static void update_highlight(struct swaylock_state *state) {
  ...
  int ri = state->ripple_idx % MAX_RIPPLES;
  state->ripple_idx++;
  clock_gettime(CLOCK_MONOTONIC, &state->ripples[ri].time);

  if (state->input_state == INPUT_STATE_LETTER) {
    if (state->xkb.caps_lock && state->args.show_caps_lock_indicator) {
      state->ripples[ri].color = state->args.colors.caps_lock_key_highlight;
    } else {
      state->ripples[ri].color = state->args.colors.key_highlight;
    }
  } else if (state->input_state == INPUT_STATE_BACKSPACE) {
    if (state->xkb.caps_lock && state->args.show_caps_lock_indicator) {
      state->ripples[ri].color = state->args.colors.caps_lock_bs_highlight;
    } else {
      state->ripples[ri].color = state->args.colors.bs_highlight;
    }
  }
}
